...
/** The registry under $HOME/.kraftwerk: what `kraftwerk ui` records, `workspaces` shows and forgets. */
describe("kraftwerk workspaces", () => {
  let dir: Awaited<ReturnType<typeof makeEmptyDir>>;
  const run = (args: string[]) => cli(dir.root, dir.home, ["workspaces", ...args]);
  ...
  it("knows nothing until a UI registered itself", async () => {
    ...
    assert.match(r.stdout, /No workspaces known yet/);
  ...
  it("stop on a workspace that is not running is a no-op", async () => {
  ...
  it("forget drops the record by name, folder name or path; unknown refs exit 2", async () => {
    ...
    assert.match(unknown.stderr, /No known workspace "nothing-like-this"/);
